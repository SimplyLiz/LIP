...
```json
{ "symbol_uri": "...", "key": "lip:agent-lock", "value": "", "author_id": "agent:local" }
```

The lock key is agent-type-specific (`lip:agent-lock`, `lip:claude-agent-lock`, …) so two different agent types can work on the same symbol without blocking each other.
...
```json
{
  "queries": [
    ...
    { "type": "annotation_get",     "symbol_uri": "lip://local/src/auth.rs#AuthService", "key": "lip:agent-lock" }
  ]
}
```
...
2. lip_batch_query       → for each URI: blast_radius + references + annotation_get("lip:fragile")
                           + annotation_get("lip:agent-lock")
                           — 10 symbols = 1 round-trip, not 40
3. lip_annotation_set    → set lip:agent-lock on each claimed symbol
```
...
lip_annotation_set("lip:agent-lock", value="") → release the lock
```
