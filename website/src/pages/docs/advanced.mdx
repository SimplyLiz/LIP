...
```bash
lip annotate get "lip://local/src/payments.rs#processCharge" "lip:agent-lock"
...
```json
{
  "queries": [
    ...
    { "type": "annotation_get", "symbol_uri": "lip://local/src/payments.rs#processCharge", "key": "lip:agent-lock" },
...
| Key | Agent type |
|-----|-----------|
| `lip:agent-lock` | Generic agents |
| `lip:claude-agent-lock` | Claude Code |
| `lip:do-not-touch` | Universal freeze (all agents respect this) |

Different agent types use different lock keys, so a generic agent and a Claude agent can work on the same symbol simultaneously without blocking each other. To block all agents, use `lip:do-not-touch`.
